...
def train_amp(agent, env, replay, reference_replay, logger, args):
    ...
    dataset_reference = agent.dataset(reference_replay.dataset)
    ...
    def train_step(tran, worker):
        for _ in range(should_train(step)):
            ...
            with timer.scope("reference_dataset"):
                reference_batch[0] = next(dataset_reference)
            outs, state[0], mets = agent.train(batch[0], state[0], reference_batch[0])
        ...
        if should_log(step):
            agg = metrics.result()
            report = agent.report(batch[0], reference_batch[0])
