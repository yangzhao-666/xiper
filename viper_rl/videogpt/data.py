...
def load_dataset(config, train, modality='video'):
    ...
    if data_type in ['mp4', 'npz']:
        dataset = get_dataset(
            config.data_path, data_type, batch_size, initial_shape=(N, batch_size // N), mask=None
        )
...
def load_npz(config, data_path, train, n_shards, shard_id, mask):
    ...
    def process(video):
        if hasattr(config, 'seq_len'):
            ...
            # Pad short videos to req_len by repeating last frame, then
            # the truncation below will produce exactly 1 chunk.
            # Videos that were originally too short get a padded chunk,
            # but this is rare and avoids tf.cond branch issues.
            pad_len = tf.maximum(req_len - tf.shape(video)[0], 0)
            padding = tf.repeat(video[-1:], pad_len, axis=0)
...
def load_mp4s(config, data_path, train, n_shards, shard_id, mask):
    ...
    if hasattr(config, 'seq_len'):
        import cv2
        req_len = 1 + (config.seq_len - 1) * config.frame_skip
        total_eps, kept_eps, total_frames, kept_frames = 0, 0, 0, 0
        for fn in fns:
            ...
            if n >= req_len:
                kept_eps += 1
                kept_frames += n
        print(f"[{split}] Episodes: {kept_eps}/{total_eps} kept "
              f"({total_eps - kept_eps} skipped, < {req_len} frames). "
              f"Frames: {kept_frames}/{total_frames} kept.")
