...
class VideoGPTRewardModel:
    ...
    def __init__(self, task: str, vqgan_path: str, videogpt_path: str,
                 ...
                 use_ot: bool=True,
                 ot_path: str='OT/checkpoints_R/mse/cheetah_run/SN_TN_64/T_8999.pth'
                 ):
        ...
        if use_ot:
            from .ot import OT
            assert ot_path != None, 'plz input pre-trained ot model path'
            self.ot_model = OT(ot_path)
        ...
        if inputs_are_codes:
            ...
            if self.use_ot:
                if self.mask is not None:
                    init_frames = init_frames * self.mask
                init_frames = np.squeeze(init_frames, axis=0)
                init_frames = self.ot_model.translate(init_frames)
                init_frames = np.expand_dims(init_frames, axis=0)
