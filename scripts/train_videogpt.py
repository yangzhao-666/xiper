...
_OrigPTCH = ocp.PyTreeCheckpointHandler
class _PatchedPTCH(_OrigPTCH):
    def __init__(self, *args, **kwargs):
        kwargs.pop('restore_with_serialized_types', None)
        super().__init__(*args, **kwargs)
ocp.PyTreeCheckpointHandler = _PatchedPTCH
...
def main():
    ...
    while iteration <= config.total_steps:
        ...
        if iteration % config.save_interval == 0 and is_master_process:
            ...
            if is_best:
                best_save_path = checkpoints.save_checkpoint(
                    ckpt_dir=ckpt_dir + '_best',
                    target=state_,
                    step=state_.step,
                    keep=1,
                    overwrite=True,
                ...
                print('Saved best checkpoint to', best_save_path)
            del state_
        iteration += 1
